...
function getBountyConfig(env = process.env) {
  ...
  return {
    ...
    teamVoteChannelId: String(env.BOUNTY_TEAM_VOTE_CHANNEL_ID || '').trim() || null,
...
async function adminChannel() {
  ...
  return {async send(payload) {
    let lastError;
    for (const id of [...new Set([cfg.teamVoteChannelId, cfg.reviewChannelId].filter(Boolean))]) {
      try {
        const target = await channel(id);
        if (!target?.send) throw new Error(`Bounty admin channel ${id} is unavailable.`);
        return await target.send(payload);
      } catch (error) { lastError = error; }
    }
    throw lastError || new Error('No Bounty admin channel is available.');
  }};
}
...
async function handleComponent(interaction) {
  ...
  if(id.startsWith('bounty_retry:')){await retryUnmatchedTransfer(interaction,id.split(':')[1]);return true;}
...
async function retryUnmatchedTransfer(interaction, unmatchedId) {
  if (!authorized(interaction) || !interaction.guildId) { await friendly(interaction, 'Team only.'); return; }
  if (!/^[1-9]\d{0,18}$/.test(unmatchedId)) { await friendly(interaction, 'Invalid transfer record.'); return; }
  await interaction.deferReply({flags: EPHEMERAL});
  try {
    const pool = resolvePool();
    const t = (await pool.query('SELECT * FROM bounty_unmatched_transfers WHERE id=$1', [unmatchedId])).rows[0];
    if (!t || t.status !== 'manual_review') { await interaction.editReply({content: 'This transfer is no longer pending manual review.'}); return; }
    if (t.token_standard !== 'erc721') { await interaction.editReply({content: 'ERC-1155 transfers still require manual handling.'}); return; }
    const candidates = (await pool.query("SELECT * FROM bounty_submissions WHERE guild_id=$1 AND chain=$2 AND contract_address=$3 AND token_id=$4 AND status='awaiting_transfer' AND expires_at>NOW() AND transfer_tx_hash IS NULL", [interaction.guildId, t.chain, t.contract_address, t.token_id])).rows;
    const matches = [];
    for (const s of candidates) {
      const links = await deps.getWalletLinks(s.guild_id, s.sender_discord_id);
      if (links.some(x => normalizeAddress(x.wallet_address) === normalizeAddress(t.source_wallet))) matches.push(s);
    }
    if (matches.length !== 1) {
      await interaction.editReply({content: 'No single safe active submission matched. For an expired submission, use /bountypool → Recover Transfer. Otherwise, have the donor submit the exact NFT link and retry. Do not resend the NFT.'}); return;
    }
    // Reuse verified recovery, including receipt/ownership checks and atomic assignment.
    const recoveryInteraction = new Proxy(interaction, {get(target, prop) {
      if (prop === 'fields') return {getTextInputValue: key => key === 'submission_id' ? String(matches[0].id) : t.tx_hash};
      if (prop === 'deferReply') return async () => {};
      const value = Reflect.get(target, prop);
      return typeof value === 'function' ? value.bind(target) : value;
    }});
    await recoverTransfer(recoveryInteraction);
  } catch (error) {
    await interaction.editReply({content: `Retry could not be completed: ${error.message}`});
  }
}
