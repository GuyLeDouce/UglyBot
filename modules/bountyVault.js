...
async function runBountyTransferCheckCore(chainName,cfg=getBountyConfig()){
  ...
  let cursor=(await pool.query(`INSERT INTO bounty_chain_cursors(chain,vault_wallet_address,last_processed_block) VALUES($1,$2,GREATEST(0,$3-$4)) ON CONFLICT(chain,vault_wallet_address) DO UPDATE SET vault_wallet_address=EXCLUDED.vault_wallet_address RETURNING *`,[chain,cfg.vaultWalletAddress,safe,chainCfg.initialScanBlocks])).rows[0],from=Number(cursor.last_processed_block)+1,matched=0,unmatched=0;
