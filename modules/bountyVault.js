...
let transferRpcFailures = 0;
let transferRetryAfter = 0;
...
function getBountyConfig(env = process.env) {
  ...
  return {
    ...
    entryContract, entryChain: String(env.BOUNTY_POOL_ENTRY_CHAIN || env.SQUIG_COLLECTION_CHAIN || env.SQUIG_CHAIN || 'ethereum').trim().toLowerCase(),
    explorerBaseUrl: String(env.ETH_EXPLORER_BASE_URL || env.ETHERSCAN_BASE_URL || 'https://etherscan.io').replace(/\/+$/, ''),
    rpcUrl: String(env.BOUNTY_ETH_RPC_URL || env.ETH_RPC_URL || env.ALCHEMY_RPC_URL || '').trim(),
...
function isTransientRpcError(error) {
  ...
  const detail = [error?.code, error?.message, error?.shortMessage, error?.error?.message,
    error?.info?.error?.message, error?.info?.responseBody].map(x => String(x || '')).join(' ');
  return [429, 502, 503, 504].includes(status) || rpcCode === -32000 ||
    /SERVER_ERROR|NETWORK_ERROR|TIMEOUT|ECONNRESET|ETIMEDOUT|Service Unavailable|connection timeout|Internal error/i.test(detail);
}
...
function validateAndParseOpenSeaUrl(value) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (contract && segments[i + 1] && /^\d+$/.test(segments[i + 1])) {
      ...
      const prior = String(segments[i - 1] || '').toLowerCase(); chain = prior && !['assets', 'item'].includes(prior) ? prior : 'ethereum'; break;
    }
  }
...
function publicPanel() {
  ...
  return { embeds:[new EmbedBuilder().setColor(0x7f39fb).setTitle('THE BOUNTY VAULT').setDescription('Community powered.\nHolder curated.\nUgly approved.\n\nDonate an NFT from another collection and help build future Squigs community prizes.').addFields(
    ...
    {name:'Important',value:`Do not send ETH. Send only the NFT associated with your submission. Do not send anything you are not prepared to donate if accepted. An accepted bounty earns **${c.acceptRewardCharm.toLocaleString()} $CHARM**.`}
...
async function handleCommand(interaction) {
  ...
  else await interaction.reply({...await poolPanel(interaction.guildId),flags:EPHEMERAL});
  return true;
}
async function poolPanel(guildId) {
  ...
  return {embeds:[new EmbedBuilder().setColor(0x7f39fb).setTitle('THE BOUNTY VAULT — DRAW POOL').addFields(
    {name:'Month',value:month,inline:true},{name:'Entry contract',value:`\`${cfg.entryContract}\``},{name:'Entries',value:String(rows.length),inline:true},{name:'Vault NFTs',value:String(vault),inline:true},{name:'Total prizes',value:String(vault+5),inline:true},{name:'Draw time',value:scheduledLocalLabel(month,cfg)},{name:'Token IDs',value:(rows.map(x=>`#${x.token_id}`).join(', ')||'None').slice(0,1024)}
...
async function handleComponent(interaction) {
  ...
  if(id==='bounty_how'){await friendly(interaction,'Submit first, transfer one NFT from a linked wallet, pass team review, then win a strict 24-hour community vote. Rejected NFTs are manually returned to the sending wallet.');return true;}
...
async function showSubmissionModal(interaction){
  ...
  const modal=new ModalBuilder().setCustomId('bounty_submit_modal').setTitle('Submit a Bounty').addComponents(
    ...
    new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('opensea').setLabel('Exact OpenSea NFT link preferred').setMaxLength(400).setRequired(true).setStyle(TextInputStyle.Short)),
...
async function createSubmission(interaction){
  ...
  if(parsed.exact && parsed.chain && !['ethereum','eth'].includes(parsed.chain)){await friendly(interaction,'The initial Bounty Vault transfer watcher supports Ethereum NFTs only.');return;}
  ...
  const cfg=getBountyConfig(), pool=resolvePool(), db=await pool.connect(); let row;
  try{await db.query('BEGIN');await db.query('SELECT pg_advisory_xact_lock($1::bigint)',[advisoryLockKey(`bounty-submit:${interaction.guildId}:${interaction.user.id}`)]);
    row=(await db.query(`INSERT INTO bounty_submissions(guild_id,sender_discord_id,project_name,opensea_url,description,chain,contract_address,token_id,status,expires_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,'awaiting_transfer',NOW()+($9||' minutes')::interval) RETURNING *`,[interaction.guildId,interaction.user.id,project,parsed.url,description||null,parsed.chain||'ethereum',parsed.contractAddress,parsed.tokenId,String(cfg.submissionTtlMinutes)])).rows[0];
    await audit(db,{guildId:interaction.guildId,submissionId:row.id,eventType:'submission_created',actorId:interaction.user.id,details:{exact:parsed.exact}});await db.query('COMMIT');
  ...
  await interaction.reply({flags:EPHEMERAL,embeds:[new EmbedBuilder().setColor(0x7f39fb).setTitle('The Bounty Vault is watching').setDescription(`Submission **#${row.id}** created for **${project}**.\n[OpenSea](${parsed.url})`).addFields({name:'Send from one of these linked wallets',value:wallets.map(x=>`\`${x}\``).join('\n').slice(0,1024)},{name:'Send exactly one NFT to',value:`\`${cfg.vaultWalletAddress}\``},{name:'Deadline',value:`<t:${Math.floor(new Date(row.expires_at).getTime()/1000)}:R>`},{name:'Reminder',value:'Do not send ETH. Send only the NFT associated with this submission.'})]});
}
async function showVault(interaction,page=0){const rows=(await resolvePool().query(`SELECT * FROM bounty_submissions WHERE guild_id=$1 AND status='vaulted' ORDER BY accepted_at,id LIMIT 10 OFFSET $2`,[interaction.guildId,page*10])).rows; const embed=new EmbedBuilder().setColor(0x7f39fb).setTitle('THE BOUNTY VAULT').setDescription(rows.length?rows.map(x=>`**${cleanText(x.project_name,100)} #${x.token_id}** — [OpenSea](${x.opensea_url})`).join('\n'):'The Vault is waiting for its first accepted bounty.'); if(rows[0]?.image_url)embed.setThumbnail(rows[0].image_url); await interaction.reply({embeds:[embed],flags:EPHEMERAL});}
...
async function runBountyTransferCheck(){
  if(Date.now()<transferRetryAfter)return {skipped:'Ethereum RPC backoff',retryAt:new Date(transferRetryAfter)};
  try{const result=await runBountyTransferCheckCore();transferRpcFailures=0;transferRetryAfter=0;return result;}
  catch(e){if(!isTransientRpcError(e))throw e;transferRpcFailures++;const retryMs=transferBackoffMs(transferRpcFailures);transferRetryAfter=Date.now()+retryMs;provider?.destroy?.();provider=null;console.warn(`[Bounty Vault] Transfer check deferred: Ethereum RPC temporarily unavailable; retrying in ${Math.ceil(retryMs/1000)}s.`);return {deferred:true,retryMs};}
}
async function runBountyTransferCheckCore(){const cfg=getBountyConfig();if(!cfg.vaultChannelId)return {skipped:'BOUNTY_VAULT_CHANNEL missing'};const p=await getProvider(),safe=Math.max(0,(await p.getBlockNumber())-cfg.minConfirmations),pool=resolvePool();let cursor=(await pool.query(`INSERT INTO bounty_chain_cursors(chain,vault_wallet_address,last_processed_block) VALUES('ethereum',$1,GREATEST(0,$2-7200)) ON CONFLICT(chain,vault_wallet_address) DO UPDATE SET vault_wallet_address=EXCLUDED.vault_wallet_address RETURNING *`,[cfg.vaultWalletAddress,safe])).rows[0],from=Number(cursor.last_processed_block)+1,matched=0,unmatched=0;const toTopic=`0x${cfg.vaultWalletAddress.slice(2).padStart(64,'0')}`;
  while(from<=safe){const to=Math.min(safe,from+999);const queries=[{topics:[TRANSFER_TOPIC,null,toTopic]},{topics:[TRANSFER_SINGLE_TOPIC,null,null,toTopic]},{topics:[TRANSFER_BATCH_TOPIC,null,null,toTopic]}];for(const q of queries){for(const log of await p.getLogs({...q,fromBlock:from,toBlock:to})){const transfers=parseInboundLog(log,cfg.vaultWalletAddress);for(const t of transfers){const r=await processInboundTransfer(t);if(r==='matched')matched++;else if(r==='unmatched')unmatched++;}}}await pool.query(`UPDATE bounty_chain_cursors SET last_processed_block=$3,updated_at=NOW() WHERE chain=$1 AND vault_wallet_address=$2`,['ethereum',cfg.vaultWalletAddress,to]);from=to+1;}return {matched,unmatched,safeBlock:safe};}
function parseInboundLog(log,vault){const topics=log.topics||[],base={chain:'ethereum',txHash:log.transactionHash,logIndex:Number(log.index??log.logIndex),blockNumber:Number(log.blockNumber),contractAddress:normalizeAddress(log.address),sourceWallet:normalizeAddress(`0x${String(topics[topics[0]===TRANSFER_TOPIC?1:2]||'').slice(-40)}`),vaultWallet:vault};if(topics[0]===TRANSFER_TOPIC&&topics.length>=4)return [{...base,tokenId:BigInt(topics[3]).toString(),tokenStandard:'erc721',quantity:'1'}];const iface=new ethers.Interface(['event TransferSingle(address indexed operator,address indexed from,address indexed to,uint256 id,uint256 value)','event TransferBatch(address indexed operator,address indexed from,address indexed to,uint256[] ids,uint256[] values)']);try{const p=iface.parseLog(log);if(p.name==='TransferSingle')return [{...base,tokenId:p.args.id.toString(),tokenStandard:'erc1155',quantity:p.args.value.toString()}];return p.args.ids.map((id,i)=>({...base,tokenId:id.toString(),tokenStandard:'erc1155',quantity:p.args.values[i].toString(),batch:true}));}catch{return [];}}
...
async function postReviewPanel(id){const s=(await resolvePool().query(`SELECT * FROM bounty_submissions WHERE id=$1`,[id])).rows[0],cfg=getBountyConfig(),ch=await channel(cfg.reviewChannelId);if(!ch)return;const embed=new EmbedBuilder().setColor(0xf2a900).setTitle('THE BOUNTY VAULT — TEAM REVIEW').addFields({name:'Project',value:cleanText(s.project_name,100)},{name:'Token',value:`#${s.token_id}`},{name:'Contract',value:`\`${s.contract_address}\``},{name:'Sender',value:`<@${s.sender_discord_id}> • \`${s.sender_wallet}\``},{name:'OpenSea',value:`[View NFT](${s.opensea_url})`},{name:'Transaction',value:`[View transaction](${cfg.explorerBaseUrl}/tx/${s.transfer_tx_hash})`},{name:'Description',value:cleanText(s.description||'None provided.',500)},{name:'Submission ID',value:String(s.id)});if(s.image_url)embed.setImage(s.image_url);const msg=await ch.send({content:cfg.reviewerUserIds.map(x=>`<@${x}>`).join(' '),embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`bounty_review_approve:${s.id}`).setLabel('Approve').setStyle(ButtonStyle.Success),new ButtonBuilder().setCustomId(`bounty_review_deny:${s.id}`).setLabel('Deny').setStyle(ButtonStyle.Danger))],allowedMentions:{users:cfg.reviewerUserIds}});await resolvePool().query(`UPDATE bounty_submissions SET team_review_channel_id=$2,team_review_message_id=$3 WHERE id=$1`,[id,msg.channelId,msg.id]);}
async function postUnmatchedWarning(t){const ch=await channel(getBountyConfig().reviewChannelId);if(ch)await ch.send({embeds:[new EmbedBuilder().setColor(0xcc3333).setTitle('UNMATCHED BOUNTY TRANSFER').setDescription('An inbound NFT could not be safely matched and remains in manual review.').addFields({name:'Contract / Token',value:`\`${t.contractAddress}\` / #${t.tokenId}`},{name:'Source wallet',value:`\`${t.sourceWallet}\``},{name:'Transaction',value:`\`${t.txHash}\``},{name:'Reason',value:t.batch?'ERC-1155 batch requires manual review':'No single safe active submission matched.'})]});}
...
async function postReturnPanel(id,reason){const s=(await resolvePool().query(`SELECT * FROM bounty_submissions WHERE id=$1`,[id])).rows[0],ch=await channel(getBountyConfig().reviewChannelId);if(!ch)return;const embed=new EmbedBuilder().setColor(0xcc3333).setTitle('BOUNTY RETURN REQUIRED').addFields({name:'NFT',value:`${cleanText(s.project_name,100)} #${s.token_id}`},{name:'Contract',value:`\`${s.contract_address}\``},{name:'Return wallet',value:`\`${s.sender_wallet}\``},{name:'Reason',value:reason},{name:'Submission ID',value:String(id)});if(s.image_url)embed.setImage(s.image_url);await ch.send({embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`bounty_return:${id}`).setLabel('Mark Returned').setStyle(ButtonStyle.Success))]});}
...
async function validateOutbound({txHash,contract,tokenId,to,quantity='1'}){if(!/^0x[0-9a-fA-F]{64}$/.test(txHash))throw new Error('Enter a valid Ethereum transaction hash.');const receipt=await (await getProvider()).getTransactionReceipt(txHash);if(!receipt||!(receipt.status===1||receipt.status==='0x1'||receipt.status===true))throw new Error('A successful receipt was not found.');const vault=getBountyConfig().vaultWalletAddress,target=normalizeAddress(to),expected=String(tokenId);for(const log of receipt.logs||[]){if(normalizeAddress(log.address)!==normalizeAddress(contract))continue;for(const t of parseInboundLog({...log,transactionHash:receipt.hash},target)){if(t.sourceWallet===vault&&t.tokenId===expected&&BigInt(t.quantity)>=BigInt(quantity))return true;}}throw new Error('The transaction does not transfer the expected NFT from the Vault to the stored wallet.');}
async function confirmOutbound(interaction,type,id){if(!authorized(interaction)){await friendly(interaction,'Team only.');return;}const tx=interaction.fields.getTextInputValue('tx_hash').trim();let row;if(type==='return')row=(await resolvePool().query(`SELECT * FROM bounty_submissions WHERE id=$1`,[id])).rows[0];else row=(await resolvePool().query(`SELECT r.*,s.contract_address,s.token_id,s.quantity,s.id submission_id FROM bounty_draw_results r JOIN bounty_submissions s ON s.id=r.bounty_submission_id WHERE r.id=$1`,[id])).rows[0];if(!row){await friendly(interaction,'This Bounty record no longer exists.');return;}try{await validateOutbound({txHash:tx,contract:row.contract_address,tokenId:row.token_id,to:type==='return'?row.sender_wallet:row.winner_wallet,quantity:row.quantity});}catch(e){await friendly(interaction,`Could not verify delivery: ${e.message} The item remains in manual review.`);return;}const pool=resolvePool(),db=await pool.connect();try{await db.query('BEGIN');if(type==='return'){const locked=(await db.query(`SELECT * FROM bounty_submissions WHERE id=$1 FOR UPDATE`,[id])).rows[0];if(locked.status!=='return_pending')throw new Error('This submission is no longer pending return.');await db.query(`UPDATE bounty_submissions SET status='returned',return_status='returned',return_tx_hash=$2,returned_by=$3,returned_at=NOW(),updated_at=NOW() WHERE id=$1`,[id,tx,interaction.user.id]);}else{const locked=(await db.query(`SELECT * FROM bounty_draw_results WHERE id=$1 FOR UPDATE`,[id])).rows[0];if(locked.delivery_status!=='pending')throw new Error('This prize is no longer pending delivery.');await db.query(`UPDATE bounty_draw_results SET delivery_status='delivered',delivery_tx_hash=$2,delivery_admin_id=$3,delivered_at=NOW() WHERE id=$1`,[id,tx,interaction.user.id]);await db.query(`UPDATE bounty_submissions SET status='delivered',updated_at=NOW() WHERE id=$1`,[row.submission_id]);}await audit(db,{guildId:interaction.guildId,submissionId:type==='return'?id:row.submission_id,eventType:type==='return'?'returned':'nft_delivered',actorId:interaction.user.id,details:{tx}});await db.query('COMMIT');}catch(e){await db.query('ROLLBACK').catch(()=>null);await friendly(interaction,e.message);return;}finally{db.release();}await interaction.reply({content:`On-chain ${type} verified and recorded.`,flags:EPHEMERAL});}

async function runBountyMonthlyDrawCheck(now=new Date()){const cfg=getBountyConfig(),current=getMonthKey(now,cfg.drawTimeZone),prior=previousMonthKey(current),guildIds=[...deps.client.guilds.cache.keys()],results=[],pool=resolvePool();for(const guildId of guildIds){const catchup=(await pool.query(`SELECT EXISTS(SELECT 1 FROM bounty_draws WHERE guild_id=$1 AND month_key=$2) OR EXISTS(SELECT 1 FROM bounty_pool_entries WHERE guild_id=$1 AND month_key=$2 AND status='active') AS due`,[guildId,prior])).rows[0]?.due;if(catchup)results.push(await generateDraw(guildId,prior));if(isMonthlyDrawDue(now,cfg))results.push(await generateDraw(guildId,current));}return {due:results.length>0,results};}
async function generateDraw(guildId,monthKey){const cfg=getBountyConfig(),pool=resolvePool(),db=await pool.connect();let draw;try{await db.query('BEGIN');await db.query('SELECT pg_advisory_xact_lock($1::bigint)',[advisoryLockKey(`bounty-draw:${guildId}:${monthKey}`)]);draw=(await db.query(`INSERT INTO bounty_draws(guild_id,month_key,scheduled_for) VALUES($1,$2,$3) ON CONFLICT(guild_id,month_key) DO UPDATE SET updated_at=NOW() RETURNING *`,[guildId,monthKey,getScheduledDrawDate(monthKey,cfg)])).rows[0];if(['generated','revealing','completed'].includes(draw.status)){await db.query('COMMIT');return draw;}const entries=(await db.query(`SELECT * FROM bounty_pool_entries WHERE guild_id=$1 AND month_key=$2 AND status='active' ORDER BY id FOR UPDATE`,[guildId,monthKey])).rows,valid=[];const contract=new ethers.Contract(cfg.entryContract,ERC721_ABI,await getProvider());for(const e of entries){try{const owner=normalizeAddress(await contract.ownerOf(BigInt(e.token_id)));if(owner)valid.push({...e,owner});}catch{}}
...
async function postDeliveryPanel(resultId){const r=(await resolvePool().query(`SELECT r.*,s.project_name,s.contract_address,s.token_id,s.quantity,s.image_url,s.opensea_url FROM bounty_draw_results r JOIN bounty_submissions s ON s.id=r.bounty_submission_id WHERE r.id=$1`,[resultId])).rows[0],ch=await channel(getBountyConfig().reviewChannelId);if(!ch)return;const embed=new EmbedBuilder().setColor(0xf2a900).setTitle('BOUNTY NFT DELIVERY').addFields({name:'Prize',value:`${cleanText(r.project_name,100)} #${r.token_id}`},{name:'Contract',value:`\`${r.contract_address}\``},{name:'Winning Squig',value:`#${r.winning_entry_token_id}`},{name:'Winner wallet',value:`\`${r.winner_wallet}\``},{name:'Discord winner',value:r.winner_discord_id?`<@${r.winner_discord_id}>`:'Unlinked wallet — manual handling required'},{name:'OpenSea',value:`[View NFT](${r.opensea_url})`});if(r.image_url)embed.setImage(r.image_url);await ch.send({embeds:[embed],components:[new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId(`bounty_deliver:${r.id}`).setLabel('Mark Delivered').setStyle(ButtonStyle.Success))],allowedMentions:{users:r.winner_discord_id?[r.winner_discord_id]:[]}});}
